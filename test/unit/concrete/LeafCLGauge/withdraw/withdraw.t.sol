...
contract WithdrawConcreteUnitTest is LeafCLGaugeTest {
        ...
        whenPenaltyRateIsZero
    {
        ...
        addRewardToLeafGauge(address(gauge), TOKEN_1);
        skip(1 days);
    ...
    modifier whenPenaltyRateIsGreaterThanZero() {
        ...
        leafGaugeFactory.setPenaltyRate({_penaltyRate: penaltyRate});
        vm.stopPrank();
        _;
    }
        ...
        whenCalledWithinMinStakeTime
    {
        ...
        addRewardToLeafGauge(address(gauge), 1209600); // rate = 2 wei/sec → earned = 1 after 1s
        skip(1); // penalty = 1 * 1 / 10_000 = 0 (rounds down)

        vm.startPrank(users.alice);
        gauge.withdraw({tokenId: tokenId});

        assertGt(leafXVelo.balanceOf(users.alice), 0);
    }
        ...
        whenThereAreNoRemainingRewardsAfterPenalty
    {
        ...
        uint256 penaltyBufferBefore = gauge.penaltyBuffer();
        vm.expectEmit(address(gauge));
        emit Withdraw({user: users.alice, tokenId: tokenId, liquidityToStake: liquidity});
        gauge.withdraw({tokenId: tokenId});

        // 100% penalty — alice gets no rewards, penalty buffered
        assertEq(leafXVelo.balanceOf(users.alice), 0);
        assertGt(gauge.penaltyBuffer(), penaltyBufferBefore);
        assertEq(nft.ownerOf(tokenId), users.alice);
        assertEqUint(pool.stakedLiquidity(), 0);
    }
        ...
        whenThereAreRemainingRewardsAfterPenalty
    {
        ...
        assertEq(leafXVelo.balanceOf(users.alice), netEarned);
        assertEq(gauge.penaltyBuffer(), expectedPenalty);
        assertEq(nft.ownerOf(tokenId), users.alice);
        assertEqUint(pool.stakedLiquidity(), 0);
    }
